...
int* indexPolygon (double* angleArray, double* sortedAngleArray, int n)  //Function to return index of polygon
{
    ...
    double angle_Array[] {0,0,0,0,0,0};
    for (int i=0;i<n;i++)
        angle_Array[i] = angleArray[i];
    ...
    {
        int j=0;
        if (sortedAngleArray[i] == angle_Array[i])
            {
                polygonIndex[i]=i;
                angle_Array[i] = -1.0;
            }
        ...
        {
            while (sortedAngleArray[i] != angle_Array[j])
                j++;
            polygonIndex[i]=j;
        }
...
*/
    ...
    cout<<"Minimum Distance of point P from all edges: "<<minDistance<<endl<<endl;
...
int main()
{
    ...
    */
    ...
    cout<<"\n\nIndex and Original MV Coordinates\n";
    for (int i=0; i<n; i++)
        cout<<i+1<<"   "<<mvOriginal[i]*100<<endl;
    cout<<endl<<endl;

    if (isInside(verticesSorted,n,p))
    {
    ...
    for (int i=0; i<n; i++)
        ...
        cout<<"Point outside Sorted Polygon\n";
